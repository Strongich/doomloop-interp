r"""Preparation functions for the math eval datasets.

All are normalized to the SAME schema so they're interchangeable downstream:
...
Datasets (structure confirmed via the HF Dataset Viewer):
  ...
  - HuggingFaceH4/MATH-500
                        split test, 500 problems over 7 subjects and 5 levels.
                        The question column is `problem`; `answer` is the final
                        value and is often SYMBOLIC (`\frac{3}{2}`, `2\sqrt{5}`,
                        an interval), unlike the integer answers of the other
                        three. Reserved for evaluating frozen, GSM8K-selected
                        policies -- never used for tuning.
"""
...
def prepare_math500() -> Dataset:
    """HuggingFaceH4/MATH-500: the question column is `problem`, so rename first.

    `subject` and `level` are dropped by `_normalize` along with every other
    original column. Recover them from the source dataset when a report needs to
    break results down that way; the common schema deliberately stays narrow.
    """
    ds = load_dataset("HuggingFaceH4/MATH-500", split="test")
    ds = ds.rename_column("problem", "question")
    return _normalize(ds, source="math500", subset="default", split="test")
